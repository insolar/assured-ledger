...
func testBackpressureBufferLimit(t *testing.T, parWriters int, hasBuffer bool, startWorker bool) {
	...
	missedFn := func(missed int) (level logcommon.Level, i []byte) {
...
func wrapOutput(output io.WriteCloser) *logwriter.Adapter {
	return logwriter.NewAdapter(output, false, nil, func() error {
		if _, ok := output.(*TestWriterStub); !ok {
			_ = output.Close()
		}
		panic("fatal")
	})
