...
func TestDeduplication_MethodUsingPrevVE(t *testing.T) {
	...
	for _, test := range table {
		t.Run(test.name, func(t *testing.T) {
			...
			expectedPublished := 1 // state request
...
func (s *deduplicateMethodUsingPrevVETest) getOutgoingRef() reference.Global {
	callee := s.getObject() // TODO: FIXME: should be caller
...
func (s *deduplicateMethodUsingPrevVETest) waitMessagePublications(
	ctx context.Context,
	t *testing.T,
	expected int,
) {
	...
	if !s.server.PublisherMock.WaitCount(expected, 10*time.Second) {
		require.Fail(t, "timeout waiting for messages on publisher")
	}
