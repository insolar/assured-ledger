...
// b.Foo calls a.Bar, pulse changes, outgoing has proper token
func TestVirtual_CallMethodOutgoing_WithPulseChange(t *testing.T) {
	...
	authService.GetCallDelegationTokenMock.Set(func(outgoing reference.Global, to reference.Global, pn pulse.Number, object reference.Global) (c1 payload.CallDelegationToken) {
		service := authentication.NewService(ctx, server.JetCoordinatorMock)
		return service.GetCallDelegationToken(
			outgoing, to, pn, object)
	})
	...
	var (
		...
		outgoingCallRef = reference.NewRecordOf(objectBGlobal, callOutgoing)
	)
	...
	// add ExecutionMocks to runnerMock
	{
		outgoingCall := execution.NewRPCBuilder(outgoingCallRef, objectBGlobal).
			CallMethod(objectAGlobal, classA, "Bar", []byte("123")).
		...
		objectBExecutionMock.AddStart(
			func(ctx execution.Context) {
				logger.Debug("ExecutionStart [B.Foo]")
				assert.Equal(t, objectBGlobal, ctx.Object)
		...
		objectBExecutionMock.AddContinue(
			func(result []byte) {
				logger.Debug("ExecutionContinue [B.Foo]")
		...
		runnerMock.AddExecutionMock("Bar").AddStart(
			func(ctx execution.Context) {
				logger.Debug("ExecutionStart [A.Bar]")
				assert.Equal(t, objectAGlobal, ctx.Request.Callee)
				assert.Equal(t, objectBGlobal, ctx.Request.Caller)
				assert.Equal(t, []byte("123"), ctx.Request.Arguments)
			},
			&execution.Update{
				Type:   execution.Done,
				Result: requestresult.New([]byte("finish A.Bar"), objectBGlobal),
			},
		)

		runnerMock.AddExecutionClassify("Bar", barIsolation, nil)
...

	{
		...
		typedChecker.VCallRequest.Set(func(request *payload.VCallRequest) bool {
			...
			assert.Equal(t, objectBGlobal, request.Caller)
			assert.Equal(t, []byte("123"), request.Arguments)
			assert.Equal(t, payload.CTMethod, request.CallType)
			assert.Equal(t, uint32(1), request.CallSequence)
			assert.Equal(t, outgoingCallRef, request.CallReason)
			assert.Equal(t, payload.BuildCallFlags(barIsolation.Interference, barIsolation.State), request.CallFlags)
			...
			service := authentication.NewService(ctx, server.JetCoordinatorMock)
			origToken := service.GetCallDelegationToken(
				outgoingCallRef, server.GlobalCaller(), server.GetPulse().PulseNumber, objectBGlobal)

			assert.Equal(t, origToken, request.DelegationSpec)
		...
		typedChecker.VCallResult.Set(func(res *payload.VCallResult) bool {
			...
			switch res.Callee {
			case objectAGlobal:
				assert.Equal(t, []byte("finish A.Bar"), res.ReturnArguments)
				assert.Equal(t, payload.CTMethod, res.CallType)
				assert.Equal(t, objectBGlobal, res.Caller)
			case objectBGlobal:
				assert.Equal(t, []byte("finish B.Foo"), res.ReturnArguments)
				assert.Equal(t, payload.CTMethod, res.CallType)
			...
			// we should resend that message only if it's CallResult from outgoing
			return res.Caller == objectBGlobal
		})
	}
