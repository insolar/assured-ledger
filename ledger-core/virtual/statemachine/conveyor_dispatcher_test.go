...
func handleFactory(_ pulse.Number, _ pulse.Range, input conveyor.InputEvent) (pulse.Number, smachine.CreateFunc) {
	switch input.(*DispatcherMessage).MessageMeta.Get("error_type") {
	case "error":
		return 0, nil
	default:
...
func newDispatcherWithConveyor() dispatcher.Dispatcher {
