...
func newAdapter(level logcommon.LogLevel) (logcommon.Logger, error) {
...
func TestLogAdapter_CallerInfoWithFunc(t *testing.T) {
	...
	s := buf.String()
	require.Contains(t, s, "adapter_test.go:60")
...
func TestLogAdapter_CallerInfo(t *testing.T) {
	...
	s := buf.String()
	require.Contains(t, s, "adapter_test.go:77")
}
...
func TestLogAdapter_Fatal(t *testing.T) {
	...
	zb := logadapter.NewBuilder(binLogFactory{}, zc, logcommon.InfoLevel)
	log, err := zb.Build()
