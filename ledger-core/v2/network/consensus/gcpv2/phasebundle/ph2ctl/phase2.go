...
import (
	...
	"github.com/insolar/assured-ledger/ledger-core/v2/network/consensus/common/lazyhead"
		...
		for {
			...
			*/
			...
			case done:
				...
				go stats.Record(ctx, metrics.Phase2Time.M(float64(time.Since(phase2StartedAt).Nanoseconds()/time.Millisecond.Nanoseconds())))
				return
			case np == nil:
				switch {
