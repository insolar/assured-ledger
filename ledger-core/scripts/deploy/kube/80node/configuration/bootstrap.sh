...
kubectl -n placeholder create secret generic node-keys
...
kubectl -n placeholder create secret generic node-keys  --from-file=/var/data/bootstrap/discovery-keys --dry-run=client -o yaml | kubectl -n placeholder apply -f -
