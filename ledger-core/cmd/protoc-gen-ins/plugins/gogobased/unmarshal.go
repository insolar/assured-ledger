...
func (p *unmarshal) field(file *generator.FileDescriptor, msg *generator.Descriptor, field *descriptor.FieldDescriptorProto, fieldname string,
	proto3, notation, onlyRaw bool,
) {
	...
	case descriptor.FieldDescriptorProto_TYPE_STRING:
		isRaw := onlyRaw || insproto.IsRawBytes(field, true)
		...
		if !isRaw {
			p.P(`if stringLen > 0 {`)
			p.In()
			p.P(`if dAtA[iNdEx] != `, int(protokit.BinaryMarker), ` {`)
			p.In()
			p.P(`return ErrExpectedBinaryMarker`, p.localName)
			p.Out()
			p.P(`}`)
			p.P(`iNdEx++`)
			p.Out()
	...
	case descriptor.FieldDescriptorProto_TYPE_BYTES:
		isCustomType := gogoproto.IsCustomType(field)

		isRaw := onlyRaw || insproto.IsRawBytes(field, isCustomType || !notation)
...
func (p *unmarshal) Generate(file *generator.FileDescriptor) {
	...
	for _, message := range file.Messages() {
		...
		if !message.IsGroup() {
			p.P(`if wireType == `, proto.WireEndGroup, ` {`)
			p.In()
		...
		for _, field := range fields {
			...
			if possiblyPacked {
				...
				p.In()
				p.field(file, message, field, fieldname, false, notation, true)
				p.Out()
				...
				p.Out()
				p.P(`}`)
				p.field(file, message, field, fieldname, proto3, notation, false)
			}
			...
			needsMapping := fieldMapNo > 0 && insproto.IsMappingForField(field, message.DescriptorProto, file.FileDescriptorProto)
			if needsMapping {
				p.P(`if preIndex != iNdEx { m.FieldMap.Put(`, int(field.GetNumber()), `, preIndex, iNdEx, dAtA) }`)
			}
		}
		p.Out()
		p.P(`default:`)
		...
		p.P(`iNdEx=preIndex`)
		p.P(`skippy, err := skipFn(dAtA[iNdEx:])`)
		p.P(`if err != nil {`)
		p.In()
		p.P(`return err`)
		p.Out()
		...
		p.P(`if skippy == 0 {`)
		p.In()
		p.P(`if skippy, err = skip`, p.localName, `(dAtA[iNdEx:]); err != nil {`)
		p.In()
		p.P(`return err`)
		p.Out()
		p.P(`}`)
	...
	var (
		...
		ErrExpectedBinaryMarker` + p.localName + ` = ` + fmtPkg.Use() + `.Errorf("proto: binary marker was expected")
	)
	`)
}
