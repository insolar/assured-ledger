...
func (p *keyLoader) Load(file string) (crypto.PrivateKey, error) {
	...
	if err != nil {
		return nil, errors.W(err, "[ Load ] Could't read private key")
	}

	signer, err := p.parseFunc(key)
	if err != nil {
		return nil, errors.W(err, "[ Load ] Could't parse private key")
	}
	return signer, nil
}
...
func readJSON(path string) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.W(err, "[ read ] couldn't read keys from: "+path)
	}
	var keys map[string]string
	err = json.Unmarshal(data, &keys)
	if err != nil {
		return nil, errors.W(err, "[ read ] failed to parse json.")
	}
